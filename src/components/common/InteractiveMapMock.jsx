...
export function DemoRoleSwitcher() {
  ...
  return (
    <div className="fixed bottom-20 sm:bottom-6 left-5 z-40">
      {isOpen ? (
        <div className="bg-slate-900 text-white rounded-2xl p-4 shadow-2xl border border-slate-700 w-64 animate-in zoom-in-95">
          <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-rose-400">ডেমো রোল পরিবর্তন</span>
            <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-white text-xs">✕</button>
          </div>
          <p className="text-xs text-slate-400 mb-3">যেকোনো একটি রোলে ক্লিক করে তৎক্ষণাৎ টেস্ট করুন:</p>
          <div className="space-y-1.5">
            <button
              onClick={() => {
                localStorage.setItem('roktokonika_user', JSON.stringify({
                  id: 'donor-demo',
                  name: 'তানভীর আহমেদ',
              ...
              className="w-full text-left px-3 py-2 rounded-xl text-xs font-semibold bg-rose-600/20 text-rose-300 hover:bg-rose-600 hover:text-white transition-all flex items-center justify-between"
            >
              <span>🩸 রক্তদাতা (Donor)</span>
              <span className="text-[10px] opacity-75">O+</span>
            </button>
            ...
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-xl text-xs font-semibold bg-sky-600/20 text-sky-300 hover:bg-sky-600 hover:text-white transition-all flex items-center justify-between"
            >
              <span>🔍 রক্ত প্রার্থী (Seeker)</span>
              <span className="text-[10px] opacity-75">B+</span>
            </button>
            ...
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-xl text-xs font-semibold bg-purple-600/20 text-purple-300 hover:bg-purple-600 hover:text-white transition-all flex items-center justify-between"
            >
              <span>🛡️ অ্যাডমিন প্যানেল (Admin)</span>
              <span className="text-[10px] opacity-75">All Access</span>
            </button>
            ...
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-xl text-xs font-medium text-slate-400 hover:bg-slate-800 hover:text-white transition-all"
            >
              👤 সাধারণ ভিজিটর (Guest Logout)
            </button>
          </div>
        </div>
      ) : (
        <button
          ...
          className="bg-slate-900/90 backdrop-blur-md text-white px-3 py-2 rounded-2xl shadow-xl border border-slate-700 text-xs font-bold flex items-center gap-2 hover:bg-slate-800 transition-all group"
        >
          <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping" />
          <span>⚡ ডেমো রোল পরিবর্তন</span>
        </button>
      )}
    </div>
  );
